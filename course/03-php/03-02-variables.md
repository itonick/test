...
>
> ただし、`str_replace()` や `trim()` は `mb_` 不要です（バイト単位でも問題なく動くため）。
...
> ```
>
> 第2引数に全角スペースを追加してください。**日本語フォームでは必須の対処です。**
...
| 全角スペースが消えない | `trim()` のデフォルト | 第2引数に全角スペースを追加 |
