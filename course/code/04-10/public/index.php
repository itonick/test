...
/**
 * 掲示板（DB版）— 第4部 総合演習の完成版
 *
 * 第3部（JSON版）の index.php とほぼ同じ構造。
 * データの保存先が JSON ファイルからデータベースに変わっただけ。
 * リポジトリの中身を差し替えたおかげで、このファイルの変更は最小限。
 */
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($action === "create") {
        try {
            ...
            // 自分の投稿として記録（削除に使う edit_token）
            $_SESSION["my_posts"][$new_id] = $post->editToken;
...
<main class="container">
    ...
    <form method="post" class="post-form">
      ...
      <div class="field">
        ...
        <input type="text" id="name" name="name"
               ...
               placeholder="<?= e(Post::DEFAULT_NAME) ?>" autocomplete="nickname">
      </div>
  ...
  <form method="get" class="search-form" role="search">
    ...
    <input type="search" id="q" name="q" value="<?= e($keyword) ?>" placeholder="投稿を検索">
  ...
  <section aria-label="投稿一覧">
    <?php if (count($posts) === 0): ?>
      <p class="empty">
        <?php if ($keyword !== ""): ?>
          条件に一致する投稿が見つかりませんでした。<br>別のキーワードでお試しください。
        <?php else: ?>
          まだ投稿がありません。<br>上のフォームから、最初の投稿をしてみてください。
        <?php endif; ?>
      </p>
    <?php else: ?>
      <ul class="post-list">
        <?php foreach ($posts as $post): ?>
          <li class="post">
            <div class="post-head">
              ...
              <?php if ($post->userName !== null): ?>
                <span class="member-badge">会員</span>
              <?php endif; ?>
              <span class="post-meta">
                <?php if ($post->commentCount > 0): ?>
                  返信 <?= $post->commentCount ?>件 ・
                <?php endif; ?>
                <?= e($post->formattedDate()) ?>
              </span>
            </div>
